...
<script type="text/javascript" charset="utf8" src="https://cdn.datatables.net/1.12.1/js/jquery.dataTables.js"></script>
    <br><br><br><br>
    <h2 class="returns_list_title">회원 목록</h2>
    <hr>
...
window.onload = function() {
   ...
   request.open("post", "/nikepro/memberList.do", true);
   ...
   request.onreadystatechange = function () {
      if (request.readyState === 4 && request.status === 200) {
         ...
         if (data === "로그인을 해주세요.") {
            // alert(data);
            swal('로그인을 해주세요', "관리자 전용 게시판입니다", 'warning');
            ...
            code += "<thead><tr><td>아이디</td><td>이름</td><td>전화번호</td><td>생년월일</td><td>마일리지</td></tr></thead>";
            code += '<tbody>';

            for (index in str) {
               code += "<tr><td>" + str[index].mem_id + "</td>";
               code += "<td>" + str[index].mem_name + "</td>";
               code += "<td>" + str[index].mem_tel + "</td>";
               code += "<td>" + str[index].mem_birth + "</td>";
               code += "<td>" + str[index].mem_mileage + "&nbsp&nbsp;";
               
              /*  if (str[index].cart_no.substr(0,1) === "U") {
                   code += "<a href= '/nikepro/reviewInsert.do?cart_no=" + str[index].mem_id +"'><input type='button' class='returns_list_table_btn' value='리뷰쓰기'></a>";
